...
export const AnalyticsManager: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div>
        <h2 className="font-serif text-2xl font-bold text-gray-900 dark:text-white">Product Click Tracking Analytics</h2>
        <p className="text-xs text-gray-500 dark:text-gray-400">Real-time user engagement tracking on product view clicks</p>
      </div>
              ...
              <div key={product.id} className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 dark:text-gray-400 font-mono">Rs {product.price.toLocaleString()}</span>
            ...
            {leastClicked.map((product) => (
              <div key={product.id} className="py-3 flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  <img src={product.images[0]} alt="" className="w-8 h-10 object-cover rounded bg-gray-100 dark:bg-gray-800" />
                  <div>
                    <span className="font-semibold text-gray-900 dark:text-gray-200 block truncate max-w-[180px]">{product.title}</span>
                    <span className="text-gray-500 dark:text-gray-400">{product.category}</span>
                  </div>
                </div>

                <div className="text-right">
                  <span className="font-mono text-gray-500 dark:text-gray-400 block">{product.clicks || 0} clicks</span>
                  <span className="text-[10px] text-rose-500 font-semibold">Consider Discount</span>
                </div>
              </div>
            ))}
          </div>
